...
const epkItems = [
  { label: "이인규블루스밴드 EPK (한국어)", desc: "밴드 소개, 셋리스트, 기술 라이더 포함", file: "/epk/ik-blues-band-epk-ko.pdf", available: false },
  { label: "이인규 개인 포트폴리오 (한국어)", desc: "솔로 바이오, 음반, 수상 이력", file: "/epk/solo-portfolio-ko.pdf", available: false },
  { label: "IK Blues Band EPK (English)", desc: "Band bio, setlist, tech rider", file: "/epk/ik-blues-band-epk-en.pdf", available: false },
];
            ...
            {epkItems.map((item, i) => (
              <div key={i} className="card p-6">
                <div className="flex items-start justify-between mb-3">
                  ...
                  {!item.available && <span className="tag text-xs">준비 중</span>}
                </div>
                ...
                {item.available ? (
                  <a href={item.file} download className="inline-block w-full text-center bg-gold hover:bg-gold-light text-black text-xs font-semibold py-2 rounded-lg transition-colors">
                    다운로드 ↓
                  </a>
                ) : (
                  <p className="text-white/20 text-xs text-center py-2">곧 업로드 예정입니다</p>
                )}
              </div>
            ))}
          </div>
