...
const featuredConcerts = concerts.filter((c) => c.featuredPersonal).slice(0, 5);
        ...
        <div className="container-lg relative z-10 py-24">
          ...
          <div className="flex flex-wrap gap-2 mt-8 animate-fade-up animate-fade-up-d3">
            {artist.highlights.slice(0, 3).map((h, i) => (
              <span key={i} className="tag-gold text-xs">
                {h}
              </span>
            ))}
          </div>
          ...
          <div className="flex gap-4 mt-10 animate-fade-up animate-fade-up-d4">
            <Link
              href="/music/ik-blues-band"
              className="bg-gold hover:bg-gold-light text-black font-semibold px-6 py-3 rounded-lg transition-colors text-sm tracking-wide"
            >
              포트폴리오 보기
            </Link>
            <Link
              href="/contact"
              className="border border-white/20 hover:border-white/50 text-white/70 hover:text-white px-6 py-3 rounded-lg transition-colors text-sm tracking-wide"
            >
              섭외 문의 →
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {activityCards.map((card) => (
              <Link
                key={card.href}
                href={card.href}
                className={`card border-2 p-6 transition-all duration-300 group ${card.accent}`}
              >
                <div className="text-3xl mb-4">{card.icon}</div>
                <p className={`text-xs tracking-widest font-medium mb-2 ${card.tagColor}`}>
                  {card.tag}
                </p>
                <h3 className="text-white font-bold text-lg mb-1 group-hover:text-gold transition-colors leading-tight">
                  {card.title}
                </h3>
                <p className="text-white/40 text-xs mb-3">{card.titleEn}</p>
                <p className="text-white/60 text-sm">{card.desc}</p>
                <p className={`mt-4 text-xs font-medium ${card.tagColor} opacity-0 group-hover:opacity-100 transition-opacity`}>
                  자세히 보기 →
                </p>
              </Link>
            ))}
              ...
              <div key={c.id} className="concert-row px-6">
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {c.category === "해외" && (
                      <span className="tag-gold">해외</span>
                    )}
                  </div>
      ...
      <section className="section bg-black text-center">
        <div className="container-lg max-w-xl mx-auto">
          ...
          <p className="text-white/50 text-sm mb-8 leading-relaxed">
            페스티벌, 행사, 라이브 클럽, 콜라보레이션 제안 등<br />
            모든 문의를 환영합니다.
          </p>
          <Link
            href="/contact"
            className="inline-block bg-gold hover:bg-gold-light text-black font-semibold px-8 py-3.5 rounded-lg transition-colors text-sm tracking-wide"
          >
            문의 페이지로 이동 →
          </Link>
        </div>
